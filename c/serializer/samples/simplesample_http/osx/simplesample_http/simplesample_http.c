...
static const char* connectionString = "HostName=cameronsHub.azure-devices.net;DeviceId=CameronPhotonDevice;SharedAccessKey=vvTA1yug0AwCmbl75mayZW2Sr6PbGPBU1/6w5yTTvQw=";
...
void simplesample_http_run(void)
{
    ...
    {
        IOTHUB_CLIENT_HANDLE iotHubClientHandle = IoTHubClient_CreateFromConnectionString(connectionString, HTTP_Protocol);
                ...
                else
                {
                    ...
                    myWeather->WindSpeed = avgWindSpeed + (rand() % 4 + 2);
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (IoTHubClient_SendEventAsync(iotHubClientHandle, messageHandle, sendCallback, (void*)1) != IOTHUB_CLIENT_OK)
                                {
                                    printf("failed to hand over the message to IoTHubClient");
                                }
                                else
